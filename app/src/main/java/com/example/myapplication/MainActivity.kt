...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun check() {
        ...
        if (firstPlayer.size == 5 && secondplayer.size == 4 && winner == 3) {
            Toast.makeText(this, "თამაში დამთავრდა ფრით", Toast.LENGTH_SHORT).show()
        }
        if (winner == 1) {
            Toast.makeText(this, "გამარჯვებულია player1", Toast.LENGTH_SHORT).show()
            score1 += 1

        }
        if (winner == 2) {
            Toast.makeText(this, "გამარჯვებულია player2", Toast.LENGTH_SHORT).show()
            score2 += 1
        }
